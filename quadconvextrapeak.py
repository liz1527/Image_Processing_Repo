...
hdr08B = fits.getheader('sem/UDS_08B_K.fits') # random year (same in all)
...
for sem in semesters:
    ...
    extras = np.load('extras/quadpeakextras'+sem+'.npy')
    ...
    for n, sigma in enumerate(sigmaold):
        sigmakernel[n] = np.sqrt(sigmabroad**2 - sigma**2 + extras[n])
    print(sigmakernel)
    
    ## Open image ###
    filename = 'sem/UDS_' + sem + '_K.fits'
...
#    print(np.size(diff[diff!=0]))
    ...
    newfilename = 'extrapeak_UDS_' + sem + '_K.fits'
