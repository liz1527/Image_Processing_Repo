...
obdata = fits.getdata('mag_flux_tables/mag_flux_table_best_H.fits', 1)
obdata = obdata[obdata['NUMBER_06B']==obnum] #restrict to just 252446
...
#    
    imdata = fits.getdata('H/UDS_'+sem+'_H.fits')
    ### Find coordinates of objects ###
    x = obdata['X_IMAGE_06B']#+sem]
    x = x.astype(int)#int(x)
    y = obdata['Y_IMAGE_06B']#'+sem]
...
ani.save('62253/62253_H_small.mp4',writer=writer)