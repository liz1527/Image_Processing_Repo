...
#Put data in array
avgFWHM = np.array([np.median(sem05B[colname]), np.median(sem07B[colname]), 
                 np.median(sem08B[colname]), np.median(sem09B[colname]), 
                 np.median(sem10B[colname]), np.median(sem11B[colname]),
                 np.median(sem12B[colname])])
...
ax.plot(t, avgFWHM, 'ro')
plt.xlabel('Semester')
plt.ylabel('FWHM (deg)')
plt.title('Average FWHM curve for unconvolved')
plt.ylim(0.00032,0.00048)
