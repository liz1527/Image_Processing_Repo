...
for k, sem in enumerate(semesters):
    ...
    tests =np.linspace(-0.15,0.65,5000)
    r = np.arange(0,42,1) * const * 3600 # define radius values
    
    aimrp = radial_profile(aimpsf, centre)
    ...
    aimphot = aperture_photometry(aimpsf, aperture)
    aimaperflux = aimphot['aperture_sum'][0]
    aperfinal = np.empty(4)
    sigmafinal = np.zeros(4)
    aperfinal[aimind] = aimaperflux
#    plt.figure(1)
...
#            vari_funcs.no_ticks()
        ...
        diff = np.empty(len(tests))
        aperflux = np.empty(len(tests))
        sigmakernel = np.empty(len(tests))
        for m, extra in enumerate(tests):
    #        print(extra)
            sigmakernel[m] = np.sqrt(sigmabroad**2 - sigma**2) + extra
            if sigmakernel[m] <= 0:
                diff[m]=99
                continue
            new = convolve_one_psf(oldpsf[quad], sigmakernel[m])    
...
#            else:
#                sumdiffold = sumdiff     
        minimum = np.argmin(np.abs(diff))
        extras[n] = tests[minimum]
        aperfinal[n] = aperflux[minimum]
        sigmafinal[n] = sigmakernel[minimum]
    print(sem + str(extras))
    print(aperfinal)
#    np.save('quadextras'+sem, extras)